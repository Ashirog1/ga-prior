//
// Created by p52 on 3/2/23.
//

#ifndef GA_PRIORITY_DELIVER_NETWORK_SIMPLEX_H
#define GA_PRIORITY_DELIVER_NETWORK_SIMPLEX_H
...
template <typename Flow = long, typename Cost = long, typename CostSum = Cost> struct network_simplex {
...
private:
  ...
  void init() {
    ...
    block_size = std::max(int(ceil(sqrt(V + 1))), std::min(10, V + 1));
  }
...
#endif // GA_PRIORITY_DELIVER_NETWORK_SIMPLEX_H
